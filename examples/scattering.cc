...
int main(int argc, char **argv) {
  ...
  std::string physListName("SCPhysicsList");

  app.add_option("-m,--macro", macroName,
                 "<Geant4 macro filename> Default: None");
  app.add_option("-p,--physlist", physListName,
                 "<Geant4 physics list macro> Default: SCPhysicsList");
  app.add_option("-s,--seed", seed,
                 "<Geant4 random number seed + offset 1234> Default: 1234");
  ...
  if (macroName.empty()) {
    G4cout << "No interactive mode running of example: provide a macro!"
           << G4endl;
    return 1;
  }
...
#ifdef G4MULTITHREADED
  nthreads = std::min(nthreads,
                      G4Threading::G4GetNumberOfCores()); // limit thread number
                                                          // to max on machine
