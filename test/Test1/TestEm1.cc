...
int main(int argc,char** argv) {
  ...
  CLI::App    app{ "Sample Secondaries Test App" };
  int         seed     = 1234;
  std::string outputFileName("secondaries.root");
  std::string macroName;
  std::string physListName("InelasticScatteringList");

  app.add_option("-m,--macro", macroName, "<Geant4 macro filename> Default: None");
  app.add_option("-p,--physlist", physListName, "<Geant4 physics list macro> Default: InelasticScatteringList");
  app.add_option("-s,--seed", seed, "<Geant4 random number seed + offset 1234> Default: 1234");
  app.add_option("-o,--outputFile", outputFileName,
                 "<FULL PATH ROOT FILENAME> Default: secondaries.root");
  ...
  auto *actions = new ActionInitialization(outputFileName, det);
